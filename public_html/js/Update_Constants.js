...
function update_omegar0_simu(){
	...
	/*
	if(omegam0>1e6){
		document.getElementById("omegam0").value=1e6.toExponential(1);
		alert("La valeur de cette constante est limitée à 1e6 pour des raisons de stabilité")
	}
	if(omegalambda0>1e6)
	*/
...
function update_omegak0_calc(){
	...
	if(Number(zld[0])>1e4){
		document.getElementById("z_lim").innerHTML="+&infin;";
		}
	else{
	document.getElementById("z_lim").innerHTML=zld[0].toFixed(4);
	}
	if(ztl[0]>1e4){
		document.getElementById("z_lim_t").innerHTML="+&infin;";
	}
	else{
