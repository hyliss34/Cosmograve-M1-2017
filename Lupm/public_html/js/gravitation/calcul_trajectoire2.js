	...
	if(r0!=0.0) {
		...
		if ( r_part < r_phy || r_part == 0) {
			...
			if(Rebond){
				...
				xi1 = posX1;
				yi1 = posY1;
				xi = x2part = scale_factor*r_part*Math.cos(phi)/rmax+canvas.width/2;
				yi = y2part = scale_factor*r_part*Math.sin(phi)/rmax+canvas.height/2;
				
				pi = (yi1-yi)/(xi1-xi);
				phi1 = phi-Math.pi()/2;
				pi1 = -pi;
				
				posX1 = yi - pi1*xi + Math.pow(pi1,2)*Math.cos(phi1);
				posY1 = yi - pi1*xi - Math.pow(pi1,2)*Math.sin(phi1);
			
				
				r_part = (posY1*rmax/(scale_factor*Math.cos(phi1))-canvas.width/2);
				phi=phi1;
