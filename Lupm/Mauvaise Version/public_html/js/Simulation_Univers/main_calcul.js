...
function calcul(){   // fonction principale de cosmogravity
	...
	if(omegalambda0_bis < OlER_max){
		...
		while (yrunge>-0.01 && yrunge<5){ // permet de boucler sur une valeur de referenc
			yrunge = rungekutta(i); //position f(x) Runge-Kutta
			if(yrunge>0){
			data_x.push(age+i/H0engannee);
			data_y.push(yrunge);
	...
	annots = [{
		...
		yref:'paper',
		yanchor:'bottom',
		text: '<b>Entrees</b><br>'+'T<sub>0</sub>:'+t0+'<br>'+'H<sub>0:</sub>'+h0+'<br>'+'Omega<sub>m0</sub>:'+omegam0+'<br>Omega<sub>L0</sub>:'+omegalambda0,
