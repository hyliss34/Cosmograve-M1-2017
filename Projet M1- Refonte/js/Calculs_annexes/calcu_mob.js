// JavaScript Document
function compteur(){document.getElementById("chargt").style.display="inline-block";}
function calcu_mob(){
	...
	// on limite la valeur de z pour le calcul de dm
	if(z1 > 1e8){zz1=1e8;}else{zz1=z1;} //<----------------------------------------------------------
	if(z2 > 1e8){zz2=1e8;}else{zz2=z2;} //<----------------------------------------------------------
	...
	if (omegak0>0){
		...
		integ_1 = Math.sqrt( Math.abs(omegak0)) * simpson(0, Number(zz1), 4e3, fonction_dm, omegam0, Number(omegalambda0), Number(Or));
		integ_2 = Math.sqrt( Math.abs(omegak0)) * simpson(0, Number(zz2), 4e3, fonction_dm, omegam0, Number(omegalambda0), Number(Or));
		...
		dm1=(c/(H0parsec) * simpson(0, Number(zz1), 4e3, fonction_dm, omegam0, Number(omegalambda0), Number(Or)));
		dm2=(c/(H0parsec) * simpson(0, Number(zz2), 4e3, fonction_dm, omegam0, Number(omegalambda0), Number(Or)));
	...
	if(omegam0 <= 0.5){
        ...
        if(z2 <= 5e3){  
                     ...
                     tempsReception = simpson(1/(1+z2), 1, 1e5, temps_parcours,Number(Or), omegam0,Number(omegak0),Number(omegalambda0)) ;//<-----------------------------A  VOIR
                     }else{
		     tempsReception = simpson(Number(z2), 5e3, 1e8, fonction_integrale, omegam0, Number(omegalambda0), Number(Or))+(1/(Math.pow(Or, 1/2)))*(1/(2*Math.pow(5e3, 2)))   };//<-----------------------
	}else{
                     if(omegalambda0 >= Olambdalim) {
                     tempsReception = simpson(0, 5e3, 1e8, fonction_integrale, omegam0, Number(omegalambda0), Number(Or))+0.5*(1/(Math.pow(Or, 1/2)))*(1/Math.pow(5e3,2)-1/Math.pow(z2,2));//<-----------------------------
                     }else{
        ...
        if(z1 <= 5e3){  
		     ...
		     tempsEmission = simpson(Number(z1), 5e3, 1e8, fonction_integrale, omegam0, Number(omegalambda0), Number(Or))+(1/(Math.pow(Or, 1/2)))*(1/(2*Math.pow(5e3, 2)))   };//<-----------------------
	}else{
                     if(omegalambda0 >= Olambdalim) {
                     tempsEmission = simpson(0, 5e3, 1e8, fonction_integrale, omegam0, Number(omegalambda0), Number(Or))+0.5*(1/(Math.pow(Or, 1/2)))*(1/Math.pow(5e3,2)-1/Math.pow(z1,2));//<-----------------------------
                     }else{
       ...
       if(Or == 0 && omegam0 != 0)      {  
        ...
        if(z2 <= 5e3){
		tempsReception = simpson(Number(z2), 5e3, 1e8, fonction_integrale, omegam0, Number(omegalambda0), 0.)+(1/(Math.pow(omegam0, 1/2)))*(2/(3*Math.pow(5e3, 3/2)));//<-----------------------
	}else{
		tempsReception = (1/(Math.pow(omegam0, 1/2)))*(2/(3*Math.pow(z2, 3/2)));//<------------------------------
	}
	if(z1 <= 5e3){
		tempsEmission = simpson(Number(z1), 5e3, 1e8, fonction_integrale, omegam0, Number(omegalambda0), 0.)+(1/(Math.pow(omegam0, 1/2)))*(2/(3*Math.pow(5e3, 3/2)));//<--------------------------
	}else{
        ...
        if(z2 <= 5e3){
		tempsReception = simpson(Number(z2), 5e3, 1e8, fonction_integrale, omegam0, Number(omegalambda0), 0.)+1/(Math.pow(omegak0, 1/2)*5e3);//<-----------------------
	}else{
		tempsReception = 1/(Math.pow(omegak0, 1/2)*z2);//<------------------------------
	}
	if(z1 <= 5e3){
		tempsEmission = simpson(Number(z1), 5e3, 1e8, fonction_integrale, omegam0, Number(omegalambda0), 0.)+1/(Math.pow(omegak0, 1/2)*5e3);//<--------------------------
	}else{
...
//les distances sont positives
	...
	document.getElementById("chargt").style.display="none";
...
function calcultheta(){window.document.getElementById("theta").value=(206265*window.document.getElementById("diametre").value/Number(dda)).toExponential(2)};// JavaScript Document