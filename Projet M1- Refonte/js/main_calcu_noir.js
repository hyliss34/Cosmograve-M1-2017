...
function calcul_noire(){   // fonction principale de cosmograve
	...
	if(omegam0 != 0 && omegaDE0 != 1){
		...
		if(Or != 0 && t0 >= 2){
			...
			age_sec = simpson(0, 5e6, 1e8, Enoire_temps, omegam0, Number(omegaDE0), Number(Or));
		}
...
function graphique_creation_noire() {
    ...
    var c = d3.scale.linear().range([0, 680]),
        ...
        f = d3.svg.axis().scale(c).orient("bottom").ticks(0).tickFormat(d3.format("d")) : d3.svg.axis().scale(c).orient("bottom").ticks(8).tickFormat(d3.format("d")),
