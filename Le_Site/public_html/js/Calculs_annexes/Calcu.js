...
function calcu(){
	...
	inf=Number(document.getElementById("z_lim_t").innerHTML) + 0.01;
        ...
        if(z2 <= 5e6){  
		     ...
		     tempsReception = simpson(inf, 5e6, fonction_integrale, Number(omegam0), Number(omegalambda0), Number(Or),Eps)+(1/(Math.pow(Or, 1/2)))*(1/(2*Math.pow(5e6, 2))) 
					 };//<-----------------------
	}else{
                     if(omegalambda0 >= Olambdalim) {
        ...
        if(z1 <= 5e6){  
		     ...
		     tempsEmission = simpson(inf, 5e6, fonction_integrale, omegam0, Number(omegalambda0), Number(Or),Eps)+(1/(Math.pow(Or, 1/2)))*(1/(2*Math.pow(5e6, 2)))   };//<-----------------------
	}else{
                     if(omegalambda0 >= Olambdalim) {
