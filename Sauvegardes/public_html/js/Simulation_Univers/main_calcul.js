...
function calcul(){   // fonction principale de cosmograve
	...
	if(omegam0 == 0 && omegalambda0 == 1){
		...
		while (yrunge2 > 0 && yrunge2 < 5.){
				if(yrunge2<0.2){pas=Math.pow(10,-4);}
	...
	// Annotations si on est pas sur mobile.
	if(window.innerWidth>960){
	annots = [{
		x: 0,
		xref:'paper',
		...
		yref:'paper',
		yanchor:'bottom',
		text: '<b>Entrees</b><br>'+'T<sub>0</sub>:'+t0+'<br>'+'H<sub>0:</sub>'+h0+'<br>'+'w<sub>m0</sub>:'+omegam0+'<br>w<sub>L0</sub>:'+omegalambda0,
 
		showarrow:false,
	},
	{"xref": "paper", "yref": "paper", "text": "<b>Sorties<\/b><br>w<sub>r0<\/sub>:0<br>w<sub>k0:<\/sub>0<br>T<sub>BB<\/sub>:1.3803e+1(Ga)", "y":1, "x": 1,xanchor:'center',yanchor:'bottom', "showarrow": false}];
	}
	else {
		annots=[];
	}
    ...
    Plotly.toImage(gd) // Ajout aux liens du data du graphique
      .then(function(url){
